...
# Force stdout to utf-8 so Unicode log chars work on Windows cp1252
...
async def log_requests(request: Request, call_next):
    ...
    if request.url.path.startswith("/api"):
        ...
        symbol = "OK" if status < 400 else "ERR"
...
async def on_startup():
    ...
    log.info(f"CORS: {len(settings.CORS_ORIGINS)} origins | docs at /docs")
